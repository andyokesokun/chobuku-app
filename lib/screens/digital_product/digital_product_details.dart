...
class DigitalProductDetails extends StatefulWidget {
  ...
  DigitalProductDetails({Key? key, required this.id}) : super(key: key);
...
class _DigitalProductDetailsState extends State<DigitalProductDetails>
  ...
  addToCart({required mode, context = null, snackbar = null}) async {
    if (is_logged_in.$ == false) {
  ...
  buildQuantityUpButton() => Container(
        ...
        child: IconButton(
            icon: FaIcon(FontAwesomeIcons.plus,
                size: 16, color: MyTheme.dark_grey),
            onPressed: () {
              if (_quantity < _stock) {
                _quantity++;
                setState(() {});
  ...
  openPhotoDialog(BuildContext context, String path) => showDialog(
        context: context,
        builder: (BuildContext context) {
          return Dialog(
            child: Container(
