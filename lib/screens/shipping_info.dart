...
class ShippingInfo extends StatefulWidget { 
  ...
  ShippingInfo({Key? key, required this.owner_id }) : super(key: key);
...
Widget buildPickupPoint(int sellerArrayIndex) {
  if (is_logged_in.$ == false) {
    return buildLoginWarning();
  }

  // Check if delivery info is being fetched or list is empty
  if (_isFetchDeliveryInfo) {
    if (_deliveryInfoList.isEmpty) {
      return buildCarrierShimmer();
    }

    // Check if seller index is valid
    if (sellerArrayIndex < 0 || sellerArrayIndex >= _deliveryInfoList.length) {
      return SizedBox.shrink();
    }

    var pickupPoints = _deliveryInfoList[sellerArrayIndex].pickupPoints;
  ...
  return SizedBox.shrink(); // fallback if nothing matches
}
...
BottomAppBar buildBottomAppBar(BuildContext context) {
  ...
  TextButton buildPickUpPointOption(BuildContext context, int sellerIndex) {
  bool isSelected = _sellerWiseShippingOption[sellerIndex].shippingOption == ShippingOption.PickUpPoint;
      ...
      children: [
        Radio<ShippingOption>(
          ...
          fillColor: WidgetStateProperty.resolveWith<Color>((states) {
            if (states.contains(WidgetState.selected)) return MyTheme.white;
        ...
        Text(
          ...
          style: TextStyle(
            ...
            color: isSelected ? MyTheme.white : MyTheme.accent_color,
          ),
        ),
      ],
    ),
...
TextButton buildAddressOption(BuildContext context, int sellerIndex) {
  bool isSelected = _sellerWiseShippingOption[sellerIndex].shippingOption == ShippingOption.HomeDelivery;
...
TextButton buildCarrierOption(BuildContext context, int sellerIndex) {
  bool isSelected = _sellerWiseShippingOption[sellerIndex].shippingOption == ShippingOption.Carrier;
